...
from src import db
from src.utils import safe_json, clean_text, sha1
from src.news import fetch_rss, fetch_newsapi, fetch_url_snippet
from src.kx_pdf import save_uploaded_pdf, extract_pdf_text, chunk_text
from src.rag import build_kx_index, kx_index_exists, search_kx
from src.scoring import score_news_item, DEFAULT_KEYWORDS, DEFAULT_ENTITIES
from src.llm import llm_available, generate_insights_llm, generate_insights_heuristic
...
st.caption("Agentic RAG: ingest → enrich(KX) → rank → explain → structured output")
...
newsapi_query = st.sidebar.text_input("NewsAPI query", value="Accenture OR artificial intelligence OR cloud regulation")
...
if results:
    ...
    with left:
        ...
        st.write(f"**Fuente:** {item.get('source','')}  ·  **Fecha:** {item.get('published_at','')}  ·  **Proveedor:** {item.get('provider','')}")
        if item.get("url"):
